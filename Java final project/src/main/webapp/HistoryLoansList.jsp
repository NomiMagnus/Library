...
<title>History loans list</title>
</head>
<body style="font-family: Segoe Print">
    ...
    <%  Iterator itr;
        List data = (List) request.getSession().getAttribute("historyData");
        if (data != null && !data.isEmpty()) {
            for (itr = data.iterator(); itr.hasNext();) {
                ...
                String none = "---";
                %>
                <tr  style="background-color: <%= isReturned ? "yellowgreen" : "tomato" %>;">
                    <td><%= loadDate %></td>
                    <td><%= returnDate %></td>
                    <td><%= userId %></td>
                    <td><%= bookId %></td>
                    <td><%= isReturned %></td>
                    <td style="background-color: <%= bgColor %>;"><%= isReturned ? none : daysLeft %></td>
                </tr>
